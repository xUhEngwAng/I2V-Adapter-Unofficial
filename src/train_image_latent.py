...
from data import LatentImageDataset
...
def sample(model, n):
    ...
    with torch.no_grad():
        for timestep in reversed(range(1, n_noise_steps)):
            t = torch.ones(n, dtype=torch.long) * timestep
            pred = model(sampled_latents, t[:, None].to(device))
...
def train(model, dataloader, args):
    ...
    for epoch in range(args.n_epoch):
        ...
        for ind, batch_images in enumerate(dataloader):
            ...
            pred = model(perturbed_images, t[:, None])
            # calculate loss
        ...
        if (epoch+1) % args.sample_epoch == 0:
            sampled_latents = sample(model, 32)
